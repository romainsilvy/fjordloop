<flux:modal name="update-housing" class="w-full max-w-4xl mt-10 max-h-[90vh]" wire:close="cleanupFields" id="update-housing-modal"
    :dismissible="false" role="dialog" aria-labelledby="update-housing-title" aria-describedby="update-housing-description">
    <div class="space-y-6" role="form" aria-labelledby="update-housing-title">
        <div>
            <flux:heading size="lg" id="update-housing-title">Modifier le logement {{ $housing?->name }}</flux:heading>
        ...
        <div class="flex">
            <flux:button wire:confirm="Êtes vous sur de vouloir supprimer ce logement ?" wire:click="delete"
                variant="danger" aria-label="Supprimer le logement {{ $housing?->name }}">Supprimer</flux:button>
